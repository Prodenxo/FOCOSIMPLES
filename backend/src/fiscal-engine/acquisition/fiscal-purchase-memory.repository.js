...
export const savePurchaseImport = async ({ invoice, items, lots }) => {
  ...
  const savedLots = lots.map((lot, idx) => {
    ...
    const saved = {
      ...
      lot_source: lot.lot_source ?? FISCAL_LOT_SOURCE.PURCHASE_XML,
      origem_mercadoria_source: lot.origem_mercadoria_source ?? ORIGEM_FISCAL_SOURCE.PURCHASE_XML_CONFIRMED,
