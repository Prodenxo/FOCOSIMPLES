...
const logNfseEmitPayloadIfDebug = (emitPayload, context = {}) => {
  ...
  console.info('[nfse-emit-debug] schema=issnet-rtc007-obra-v5 payload final (redacted):', JSON.stringify({
    ...context,
...
const validatePayload = (payload) => {
  ...
  const tomadorEnderecoObra = payload?.tomador?.endereco ?? null;
  for (const item of servicos) {
    if (!requiresNfseObraForServicoCodigo(item?.codigo)) continue;
    const obraEndereco = resolveNfseObraEndereco(item, payload, tomadorEnderecoObra);
    const obraEnderecoError = validateNfseObraEndereco(obraEndereco);
    if (obraEnderecoError) {
      throw badRequest(obraEnderecoError);
    }
  }
