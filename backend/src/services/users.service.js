...
const ROLE_UPDATE_ALLOWED_SUPERADMIN = new Set([
  "admin",
  "usuario",
  "outsider",
]);
...
export const ensureRoleId = async (adminClient, role) => {
  const resolved = await findRoleByCandidates(
    adminClient,
    getRoleCandidates(role),
  );
  if (resolved?.roleId) return resolved;

  const fallback = await findRoleByCandidates(
    adminClient,
    getRoleCandidates(ROLE_DEFAULT),
  );
  if (
    fallback?.roleId &&
    normalizeRoleValue(role) &&
    normalizeRoleValue(role) !== ROLE_DEFAULT
  ) {
    console.warn("[Users] role fallback:", {
      requestedRole: role,
...
const normalizeLegacyMeiSlotsPixInput = (
  value,
  fieldName = "legacy_mei_slots_pix",
) => {
...
const buildEmpresaPayload = (input = {}, { requireName = false } = {}) => {
  ...
  if (Object.prototype.hasOwnProperty.call(input, "legacy_mei_slots_pix")) {
    payload.legacy_mei_slots_pix = normalizeLegacyMeiSlotsPixInput(
      input.legacy_mei_slots_pix,
    );
  }

  if (requireName) {
    payload.empresa = resolveEmpresaName({
      ...input,
      empresa: payload.empresa ?? input?.empresa,
    });
...
const countActiveUsersByMei = async (
  adminClient,
  { empresaId, mei, ignoreUserId },
) => {
...
const sumActiveStripeMeiSlotsByEmpresa = async (adminClient, empresaIds) => {
  ...
  for (const row of lines || []) {
    ...
    sumByEmpresa.set(
      id,
      (sumByEmpresa.get(id) || 0) + Number(row.mei_slots || 0),
    );
  }
  return sumByEmpresa;
};
...
export const reconcileMeiModuleConsistency = async (
  ...
  const uniqueIds = Array.from(
    new Set((scopedEmpresaIds || []).filter(Boolean)),
  );
  ...
  const stripeByEmpresa = await sumActiveStripeMeiSlotsByEmpresa(
    adminClient,
    uniqueIds,
  );
  const details = [];
...
const syncEmpresasMeiActivation = async (
  adminClient,
  scopedEmpresaIds = [],
) => {
  const result = await reconcileMeiModuleConsistency(
    adminClient,
    scopedEmpresaIds,
  );
 ...
 */
const mergeStripeContractedMeiIntoEmpresaLimits = async (
  adminClient,
  empresas,
) => {
  const list = empresas || [];
...
export const ensureEmpresaCapacity = async (
  adminClient,
  { empresaId, mei, ignoreUserId },
) => {
  ...
  const total = await countActiveUsersByMei(adminClient, {
    empresaId,
    mei,
    ignoreUserId,
  });
 ...
 * @param {string} userId
 */
export const assertUserEligibleForEmpresaInvite = async (
  adminClient,
  userId,
) => {
  ...
  if (profileRole === "admin" || profileRole === "superadmin") {
    throw forbidden(
      "Contas administrativas não podem aceitar convite de usuário",
    );
  }
...
export const getRequesterContext = async (
  accessToken,
  preverifiedUser = null,
) => {
  ...
  if (!user?.id) {
    if (isLocalAuthMode()) {
      ...
      const { data: { user: fetched } = {}, error: userError } =
  ...
  if (linkError) {
    console.warn(
      "[Users] role_x_user_x_empresa lookup error:",
      linkError.message,
    );
  ...
  if (linkData?.roles_id) {
    ...
    if (roleData?.roles) {
      if (
        linkData.empresas_id &&
        (linkData.mei === true || linkData.mei === null)
      ) {
...
export const listUsers = async (accessToken, queryParams = {}) => {
  ...
  const links = await fetchAllEmpresaUserLinks(adminClient, {
    role,
    empresaId,
  });
  ...
  const userMap = await buildAuthUserMapForIds(
    adminClient,
    userIdsToFetch,
    allAuthUsers,
  );
  ...
  const roleIds = Array.from(
    new Set((links || []).map((l) => l.roles_id).filter(Boolean)),
  );
  const empresaIds = Array.from(
    new Set((links || []).map((l) => l.empresas_id).filter(Boolean)),
  );
  ...
  const [{ data: rolesData }, { data: empresasData }] = await Promise.all([
    adminClient
      .from("roles")
      .select("id, roles")
      .in("id", roleIds.length ? roleIds : ["none"]),
    adminClient
      .from("empresas")
  ...
  let resultUsers = (links || [])
    .map((link) => {
      ...
      const roleLabel = normalizeRoleValue(
        roleMap.get(link.roles_id) || (link.isOrphan ? "N/A" : "usuario"),
      );
      ...
      if (
        searchTerm &&
        !matchesUserSearch(user, { empresaName, roleLabel }, searchTerm)
      ) {
        return null;
      }
...
export const listEmpresas = async (accessToken) => {
  ...
  let query = adminClient
    .from("empresas")
    .select(
      "id, empresa, nome_fantasia, max_mei, max_usuarios_nao_mei, legacy_mei_slots_pix",
    )
  ...
  const fixedMaxMeiByEmpresa = await syncEmpresasMeiActivation(
    adminClient,
    scopedIds,
  );
  ...
  if (role === "superadmin") {
    empresas = await mergeStripeContractedMeiIntoEmpresaLimits(
      adminClient,
      empresas,
    );
  }
...
export const getEmpresa = async (accessToken) => {
  ...
  if (!ROLE_CREATE_ALLOWED.has(role)) throw forbidden();
  if (!empresaId)
    throw badRequest("Empresa nao encontrada para o usuario atual");
...
export const getEmpresaById = async (accessToken, empresaId) => {
  ...
  if (
    ...
    requester.empresaId !== normalizedEmpresaId
  ) {
...
export const updateEmpresa = async (accessToken, empresaId, input) => {
  ...
  return {
    empresa: { ...data, product_line: deriveEmpresaProductLine(data.max_mei) },
  };
};

export const createUser = async (accessToken, input, deps = {}) => {
  const getRequesterContextFn =
    deps.getRequesterContextFn || getRequesterContext;
  const createSupabaseClientFn =
    deps.createSupabaseClientFn || createSupabaseClient;
  const { role: requesterRole, empresaId: requesterEmpresaId } =
    await getRequesterContextFn(accessToken);
  ...
  if (requesterRole === "admin") {
    ...
    const normalizedRequested =
      normalizeRoleValue(requestedRole) || ROLE_DEFAULT;
  ...
  if (isLocalAuthMode()) {
    ...
    const roleDb = finalRole === "usuario" ? "usuario" : finalRole;
  ...
  const { roleId, role: resolvedRole } = await ensureRoleId(
    adminClient,
    finalRole,
  );
...
export const updateUser = async (accessToken, userId, input) => {
  ...
  if (requester.role === "admin") {
    if (!requester.empresaId || requester.empresaId !== linkRecord.empresas_id)
      throw forbidden();
    if (!ROLE_ADMIN_MANAGEABLE.has(targetRole)) throw forbidden();
    if (requestedRole && !ROLE_ADMIN_MANAGEABLE.has(requestedRole))
      throw forbidden();
  }

  if (requester.role === "superadmin") {
    if (!isSelfUpdate && !ROLE_UPDATE_ALLOWED_SUPERADMIN.has(targetRole))
      throw forbidden();
    if (
      !isSelfUpdate &&
      requestedRole &&
  ...
  if (!capacityChecked) {
    const isSelfMeiRemoval =
      isSelfUpdate && requestedMei === false && currentMeiSlot;
    const shouldCheckCapacity =
      !isSelfMeiRemoval &&
      (targetEmpresaId !== currentEmpresaId ||
        targetMeiSlot !== currentMeiSlot);
  ...
  if (isLocalAuthMode()) {
    ...
    if (Object.keys(metaPatch).length > 0 || requestedEmail || requestedPhone) {
      ...
      if (
        requestedEmail &&
        requestedEmail !== String(current.email || "").toLowerCase()
      ) {
      ...
      const nextMeta = {
        ...
        typeof current.raw_user_meta_data === "object"
          ? current.raw_user_meta_data
          : {}),
        ...metaPatch,
      };
  ...
  if (requestedDisplayName || requestedPhone) {
    ...
    if (updateUserError) {
      console.warn(
        "[Users] updateUser metadata error:",
  ...
  if (requestedEmail) {
    const { data: currentAuthUser, error: getUserError } =
      await adminClient.auth.admin.getUserById(userId);
    if (getUserError) {
      console.warn(
        "[Users] updateUser getUserById error:",
        getUserError.message,
      );
      throw badRequest(getUserError.message);
    }
    const currentEmail =
      currentAuthUser?.user?.email?.trim().toLowerCase() || "";
  ...
  if (isLocalAuthMode()) {
    ...
    await query(
      `UPDATE public.users
       ...
       WHERE id = $2`,
      [
        status ? null : new Date("2099-12-31T23:59:59.000Z").toISOString(),
        userId,
      ],
    );
  ...
  if (isLocalAuthMode()) {
    ...
    if (!isOrphanAccount) {
      ...
      if (requester.role === "admin") {
        ...
        if (
          !requester.empresaId ||
          requester.empresaId !== linkData.empresas_id
        )
          throw forbidden();
      }
    ...
    await query(`DELETE FROM public.role_x_user_x_empresa WHERE user_id = $1`, [
      userId,
    ]);
...
export const deleteEmpresa = async (accessToken, empresaId) => {
  ...
  if (isLocalAuthMode()) {
    await query(
      `DELETE FROM public.role_x_user_x_empresa WHERE empresas_id = $1`,
      [empresaId],
    );
  ...
  if (linksError)
    throw badRequest(`Erro ao remover vínculos: ${linksError.message}`);
  ...
  if (empresaError)
    throw badRequest(`Erro ao remover empresa: ${empresaError.message}`);
...
export const resetUserPassword = async (accessToken, userId, input) => {
  ...
  const { adminClient } = await getPasswordResetAuthorization(
    accessToken,
    userId,
  );
